...
a fork somewhere else by editing those two constants. Nothing is sent while `NODE_ENV` is
`test`, so a test run cannot emit.

## The shape of it

- **Server side only.** `posthog-node` in `apps/api` and `apps/agent`. There is no `posthog-js`
  in this repo, no autocapture, no session replay and nothing at all in `apps/app`. Autocapture
  on a CRM would lift contact names, email addresses, company domains and deal amounts out of
  your database; the simplest way not to hold that is not to collect it.
